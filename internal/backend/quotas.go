...
func parseQuotaBucketResult(payload map[string]any) (quotaBucketResult, error) {
	...
	if quotaLimitReached(payload) {
		result = zeroQuotaLimitedFiveHour(result)
	}
	return normalizeQuotaBucketResult(result), nil
}

func quotaLimitReached(payload map[string]any) bool {
	rateLimit, _ := payload["rate_limit"].(map[string]any)
	if limitReached, ok := boolFromMap(rateLimit, "limit_reached"); ok && limitReached {
		return true
	}
	if allowed, ok := boolFromMap(rateLimit, "allowed"); ok && !allowed {
		return true
	}
	return false
}

func zeroQuotaLimitedFiveHour(result quotaBucketResult) quotaBucketResult {
	if result.fiveHour == nil {
		return result
	}
	result.fiveHour = &quotaBucketValue{
		remainingPercent: 0,
		resetAt:          stringOr(result.weeklyResetAt(), result.fiveHour.resetAt),
	}
	return result
}

func (result quotaBucketResult) weeklyResetAt() string {
	if result.weekly == nil {
		return ""
	}
	return result.weekly.resetAt
}
