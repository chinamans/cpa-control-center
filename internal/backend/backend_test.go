...
func TestGetCachedCodexQuotaSnapshotLoadsPersistedSnapshot(t *testing.T) {
	service, err := New(t.TempDir(), nil)
	if err != nil {
		t.Fatalf("New backend: %v", err)
	}
		...
		Accounts: []CodexQuotaAccountDetail{
			{
				...
				Provider:  "codex",
				Success:   true,
				FetchedAt: "2026-05-15T09:29:02Z",
				FiveHour:  QuotaBucketDetail{Supported: true, RemainingPercent: float64Ptr(0)},
				Weekly:    QuotaBucketDetail{Supported: true, RemainingPercent: float64Ptr(42)},
			},
		},
	...
	if err := service.store.SaveCodexQuotaSnapshot(saved); err != nil {
		t.Fatalf("SaveCodexQuotaSnapshot: %v", err)
	}

	loaded, err := service.GetCachedCodexQuotaSnapshot()
	if err != nil {
		t.Fatalf("GetCachedCodexQuotaSnapshot: %v", err)
	}
	if loaded.FetchedAt != saved.FetchedAt || loaded.Source != "quota_refresh" || len(loaded.Accounts) != 1 {
		t.Fatalf("unexpected cached snapshot: %+v", loaded)
	}
}
...
func TestParseQuotaBucketResultZeroesFiveHourWhenWeeklyIsEmpty(t *testing.T) {
	payload := map[string]any{
		"plan_type": "plus",
		"rate_limit": map[string]any{
			"primary_window": map[string]any{
	...
	if result.fiveHour == nil || result.fiveHour.remainingPercent != 0 {
		t.Fatalf("expected weekly exhaustion to zero five-hour remaining, got %+v", result.fiveHour)
	}
	if result.fiveHour.resetAt != "2026-05-12T06:06:21Z" {
	...
	if result.weekly == nil || result.weekly.remainingPercent != 0 {
		t.Fatalf("expected 100%% used to leave 0%% remaining, got %+v", result.weekly)
	}
}

func TestParseQuotaBucketResultZeroesFiveHourWhenLimitReached(t *testing.T) {
	payload := map[string]any{
		"plan_type": "plus",
		"rate_limit": map[string]any{
			"allowed":       true,
	...
	if result.fiveHour == nil || result.fiveHour.remainingPercent != 0 {
		t.Fatalf("expected overall limit signal to zero five-hour remaining, got %+v", result.fiveHour)
	}
}

func TestParseQuotaBucketResultConvertsFractionalUsedPercent(t *testing.T) {
	payload := map[string]any{
		"plan_type": "plus",
		"rate_limit": map[string]any{
			"primary_window": map[string]any{
	...
	if result.fiveHour == nil || result.fiveHour.remainingPercent != 75 {
		t.Fatalf("expected fractional 0.25 used to leave 75%% remaining, got %+v", result.fiveHour)
	}
}
