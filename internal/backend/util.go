...
var quotaValuePlanOrder = []string{"free", "plus", "pro", "team", "business", "enterprise"}
...
func defaultQuotaValueByPlan() QuotaValueByPlan {
	defaults := QuotaValueByPlan{}
	for _, plan := range quotaValuePlanOrder {
		defaults[plan] = QuotaValuePlanSetting{}
	}
	defaults["plus"] = QuotaValuePlanSetting{
		FiveHour: 25,
		Weekly:   100,
	}
	return defaults
...
func normalizeQuotaValueByPlan(input QuotaValueByPlan) QuotaValueByPlan {
	values := defaultQuotaValueByPlan()
	for plan, setting := range input {
		normalizedPlan := strings.ToLower(strings.TrimSpace(plan))
		if normalizedPlan == "" {
			continue
		}
		if setting.FiveHour < 0 {
			setting.FiveHour = 0
		}
		if setting.Weekly < 0 {
			setting.Weekly = 0
		}
		values[normalizedPlan] = setting
	}
	return values
}
