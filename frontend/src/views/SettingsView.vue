...
import { quotaValuePlanKeys } from '@/utils/settings'
...
const quotaValuePlans = quotaValuePlanKeys
        ...
        <section class="settings-schedule settings-quota">
          ...
          <p class="muted">{{ t('settings.quotaUnlimitedHint') }}</p>
            ...
            <div class="settings-toggle-group">
	          ...
	          <div class="settings-quota-values">
	            <div class="settings-toggle-group">
	              <span class="form-label-with-info">
	                <span>{{ t('settings.quotaValueByPlan') }}</span>
	                <el-popover trigger="click" placement="top-start" :width="320" popper-class="settings-info-popover">
	                  <template #reference>
	                    <button type="button" class="info-trigger" :aria-label="infoAriaLabel">i</button>
	                  </template>
	                  <div class="settings-info-popover__content">
	                    <strong>{{ t('settings.quotaValueByPlan') }}</strong>
	                    <p>{{ infoText('quotaValueByPlan') }}</p>
	                  </div>
	                </el-popover>
	              </span>
	              <div class="settings-quota-value-grid">
	                <div v-for="plan in quotaValuePlans" :key="plan" class="settings-quota-value-row">
	                  <strong>{{ t(`settings.quotaPlan${plan.slice(0, 1).toUpperCase()}${plan.slice(1)}`) }}</strong>
	                  <el-form-item :label="t('settings.quotaValueFiveHour')" :error="settingsStore.errors[`quotaValueByPlan.${plan}`]">
	                    <el-input-number v-model="settingsStore.settings.quotaValueByPlan[plan].fiveHour" :min="0" :max="1000000" :precision="2" />
	                  </el-form-item>
	                  <el-form-item :label="t('settings.quotaValueWeekly')" :error="settingsStore.errors[`quotaValueByPlan.${plan}`]">
	                    <el-input-number v-model="settingsStore.settings.quotaValueByPlan[plan].weekly" :min="0" :max="1000000" :precision="2" />
	                  </el-form-item>
	                </div>
	              </div>
	            </div>
